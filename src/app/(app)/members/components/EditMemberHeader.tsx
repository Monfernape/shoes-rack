...
export const EditMemberHeader = ({id} : {id:string}) => {
  ...
  useEffect(() => {
    ...
    if (!debounceValue) {
      router.push(shouldNavigateToMembers ? Routes.Members : `${Routes.EditMember}/${id}`); 
    } else {
      ...
      <header className="bg-white shadow-sm sticky top-0">
        <div className="flex items-center justify-between px-4 py-4">
          <div className="flex items-center">
            ...
            {(!isTitleHide || !isSmallScreen) && <EditMemberBreadCrumbs id = {id} />}
          </div>
          {pathname !== `${Routes.EditMember}/${id}` && pathname !== Routes.Login && (
